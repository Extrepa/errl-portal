...
- Deploy: verified `npm test -- tests/errl-phone-controls.spec.ts` (31 passed), `npm run portal:build`; pushed `main` `2caf603` (moving edge goals) for Cloudflare Pages.
- Gates legibility + compact score HUD: Classic Throw gates inset/safe-area, larger clamps, thicker frame + `::after` aperture cue, patrol bounds + hit % tweaks (`styles.css`); RB HUD tightened `--compact` + `--micro`, narrow viewport abbreviates labels CT/Life/Col (`portal-app.js`). Verified `npm test -- tests/errl-phone-controls.spec.ts` (31 passed), `npm run portal:build`.
- Minimal dashed gates pass: simplified `#rbClassicGoals` DOM to hit-lane elements only and replaced portal-frame visuals with fixed subtle dashed edge lanes (no veil/glow/patrol), keeping `[data-rb-classic-goal]` scoring logic unchanged. Verified `npm test -- tests/errl-phone-controls.spec.ts` (31 passed), `npm run portal:build`.
