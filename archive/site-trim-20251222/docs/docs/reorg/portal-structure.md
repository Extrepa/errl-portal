...
| `src/apps/static/pages/` | Standalone HTML experiences | Each subdirectory (about, gallery, assets, games, events, merch, dev, studio, pin-designer, etc.) maps to a Vite multi-page entry declared inside `vite.config.ts`. Only ship assets that those pages import directly. |
...
4. **Standalone demo/page** → scaffold under `src/apps/static/pages/<name>/` and add an entry inside `vite.config.ts`.
...
- A tiny inline script rewrites those attributes to `/apps/static/pages/...` in dev mode or `/portal/pages/...` in production. This keeps the portal pages accessible (dev + prod) without hard-coding the prefix multiple times.
- React wrappers use `src/apps/studio/src/app/utils/portalPaths.ts` to do the same for Math Lab/Shape Madness/Pin Designer. Prefer the helper (or the `data-portal-link` pattern) whenever you add new links.

## Portal pages overview

| Page | Linked from landing? | Status / Notes |
| --- | --- | --- |
| `apps/static/pages/about` | ✅ | Main "About Errl" bubble; part of primary nav. |
| `apps/static/pages/gallery` | ✅ | Gallery bubble + hero CTA. |
| `apps/static/pages/assets` | ✅ | Assets bubble; still referenced in nav orbit. |
| `apps/static/pages/pin-designer` | ✅ | Pin Designer bubble; hosts the enamel designer. |
| `apps/static/pages/studio` | 🔁 (via `/studio` hub) | Legacy HTML experiences (Math Lab, Shape Madness, SVG Colorer, Pin Widget). React hub wraps these via iframes/routes. |
| `apps/static/pages/dev` | ❌ | Only reachable via direct URL/debug link. Keep for now (dev controls) but consider gating behind query param or promoting if still useful. |
| `apps/static/pages/events` | ✅ | Events page; linked in navigation. |
| `apps/static/pages/merch` | ✅ | Merch page; linked in navigation. |
| `apps/static/pages/games` | ✅ | Games page; hidden bubble triggered via keyboard. |
| `apps/static/pages/studio/math-lab` | ✅ (via Studio) | Loaded from Studio menu. |
| `apps/static/pages/studio/shape-madness` | ✅ (via Studio) | Linked from Studio. |
| `apps/static/pages/studio/pin-widget` | ✅ (via Studio) | Designer used by Studio; now cleaned up with only necessary assets. |
| `apps/static/pages/studio/svg-colorer` | ✅ (via Studio) | Linked from Studio experiments. |
