...
function step() {
  ...
  for (let i = nodes.length - 1; i >= 0; i -= 1) {
    ...
    const baseRadius = settings.glow * (1.2 + (i / settings.trailLength) * 1.8);
    ...
    gradient.addColorStop(0, `hsla(${node.hue}, 100%, 75%, ${1.2 * life})`);
    gradient.addColorStop(0.3, `hsla(${(node.hue + 30) % 360}, 95%, 65%, ${0.8 * life})`);
    gradient.addColorStop(0.6, `hsla(${(node.hue + 60) % 360}, 90%, 55%, ${0.4 * life})`);
    gradient.addColorStop(1, `hsla(${(node.hue + 90) % 360}, 85%, 50%, 0)`);
