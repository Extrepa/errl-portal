...
(function() {
  ...
  const HueController = {
    ...
    applyLayerCSS(layer){ if(!this.cssEnabled) return; const def = LAYERS[layer]; if(!def || !def.selectors) return; const st = this.layers[layer]; for(const sel of def.selectors){ document.querySelectorAll(sel).forEach(el=>{ el.style.filter = st.enabled ? this.layerFilterString(st) : ''; }); } },
