/**
 * Errl Battle System
 * Integrates into the main portal: click Errl's face 3 times to trigger battle mode
 * Eyes and mouth fly toward cursor as "drippy stars"
 */
...
const ErrlBattle = {
  state: {
    ...
    shards: {},
    battleStartTime: 0,
    battleDuration: 60000, // 60 seconds
    battleTimer: null,
    intensity: 1 // escalates over time
  },

  shardConfigs: {
    eyeL: { id: 'errl-eye-left', baseX: 285, baseY: 480, aggressive: true },
    eyeR: { id: 'errl-eye-right', baseX: 545, baseY: 460, aggressive: true },
    mouth: { id: 'errl-mouth', baseX: 415, baseY: 750, aggressive: true }
  },
  ...
  init() {
    ...
    if (!errlWrapper) {
      console.warn('ErrlBattle: Could not find #errl element');
      return;
    }
    console.log('ErrlBattle: Found Errl element, setting up...');

    // Make Errl clickable
    errlWrapper.style.cursor = 'pointer';
    errlWrapper.addEventListener('click', this.handleClick.bind(this));

    // Track mouse
    document.addEventListener('mousemove', this.handleMouseMove.bind(this));
    document.addEventListener('mouseleave', this.handleMouseLeave.bind(this));

    // Create a click counter display
    this.createClickCounter();

    // Create SVG overlay for shards
    this.createShardOverlay(errlWrapper);
  ...
  createClickCounter() {
    ...
    `;
    counter.textContent = 'CLICKS: 0/3';
    document.body.appendChild(counter);
  },

  updateClickCounter() {
    const counter = document.getElementById('errl-click-counter');
    if (counter) {
      counter.textContent = `CLICKS: ${this.state.clicks}/3`;
      if (this.state.isCharged) {
        counter.style.color = 'rgba(255, 100, 100, 0.8)';
        counter.style.textShadow = '0 0 10px rgba(255, 100, 100, 0.4)';
      } else if (this.state.isBattle) {
        counter.style.color = '#ff0066';
        counter.style.textShadow = '0 0 15px rgba(255, 0, 102, 0.6)';
      } else {
        counter.style.color = 'rgba(220, 220, 220, 0.4)';
        counter.style.textShadow = 'none';
      }
    }
  },

  createShardOverlay(container) {
    // Create SVG overlay that sits on top of Errl
    const svg = document.createElementNS('http://www.w3.org/2000/svg', 'svg');
    svg.setAttribute('id', 'errl-battle-overlay');
    svg.setAttribute('viewBox', '-122.76 -122.76 1725.52 2291.52');
    ...
    mouth.style.cssText = 'transform-origin: 467px 994px; transition: transform 0.08s ease-out;';
    this.state.shards.mouth = mouth;
  ...
  chargeEffects() {
    const errlWrapper = document.getElementById('errl');
    if (!errlWrapper) return;

    // Add glow to Errl
    errlWrapper.style.filter = 'drop-shadow(0 0 40px rgba(255, 100, 100, 0.5))';

    // Pulse shards
    Object.values(this.state.shards).forEach(shard => {
      shard.style.animation = 'errl-pulse-glow 0.6s ease-in-out infinite';
    });
  },
  ...
  activateBattle() {
    ...
    // Screen shake
    document.body.style.animation = 'errl-shake 0.3s ease-in-out';
    setTimeout(() => {
      document.body.style.animation = '';
    }, 300);

    // Update visuals
    errlWrapper.style.filter = 'drop-shadow(0 0 60px rgba(255, 0, 100, 0.8))';

    Object.values(this.state.shards).forEach(shard => {
      shard.style.animation = 'none';
      shard.setAttribute('fill', '#ff0066');
    });

    // Create battle timer display
    this.createBattleTimer();

    // Set auto-end timer
    this.state.battleTimer = setTimeout(() => {
      this.endBattle();
  ...
  createBattleTimer() {
    let timerDisplay = document.getElementById('errl-battle-timer');
    if (!timerDisplay) {
      timerDisplay = document.createElement('div');
      timerDisplay.id = 'errl-battle-timer';
    ...
    // Create escape button if it doesn't exist
    if (!document.getElementById('errl-escape-btn')) {
      const escapeBtn = document.createElement('button');
      escapeBtn.id = 'errl-escape-btn';
      ...
      escapeBtn.addEventListener('mouseenter', () => {
        escapeBtn.style.background = 'rgba(255, 0, 102, 0.3)';
        escapeBtn.style.textShadow = '0 0 10px rgba(255, 0, 102, 0.6)';
      });
      escapeBtn.addEventListener('mouseleave', () => {
        escapeBtn.style.background = 'rgba(255, 0, 102, 0.15)';
        escapeBtn.style.textShadow = '0 0 5px rgba(255, 0, 102, 0.3)';
      });
      escapeBtn.addEventListener('click', () => this.endBattle());
      timerDisplay.appendChild(escapeBtn);
    }
  },

  updateBattleTimer() {
    if (!this.state.isBattle) return;
    const elapsed = Date.now() - this.state.battleStartTime;
    const remaining = Math.max(0, Math.ceil((this.state.battleDuration - elapsed) / 1000));
    const timerDisplay = document.getElementById('errl-battle-timer');
    if (timerDisplay) {
      timerDisplay.textContent = `BATTLE: ${remaining}s`;
      // Color intensifies as time runs out
      if (remaining <= 10) {
        timerDisplay.style.color = '#ff3366';
        timerDisplay.style.textShadow = '0 0 20px rgba(255, 51, 102, 0.8)';
      } else if (remaining <= 20) {
        timerDisplay.style.color = '#ff0066';
        timerDisplay.style.textShadow = '0 0 15px rgba(255, 0, 102, 0.6)';
      }
    }
    // Escalate intensity
    this.state.intensity = 1 + (elapsed / this.state.battleDuration) * 0.5;
  },
  ...
  endBattle() {
    ...
    clearTimeout(this.state.battleTimer);

    const errlWrapper = document.getElementById('errl');
    if (errlWrapper) {
      errlWrapper.style.filter = '';
    }

    // Reset shards to idle
    Object.values(this.state.shards).forEach(shard => {
      shard.setAttribute('fill', '#000000');
      shard.style.transform = 'translate(0, 0) rotate(0deg)';
    });

    // Remove timer display and escape button
    const timerDisplay = document.getElementById('errl-battle-timer');
    if (timerDisplay) {
      timerDisplay.style.opacity = '0';
      setTimeout(() => timerDisplay.remove(), 300);
    }
    const escapeBtn = document.getElementById('errl-escape-btn');
    if (escapeBtn) {
      escapeBtn.remove();
    }
    ...
    const counter = document.getElementById('errl-click-counter');
    if (counter) {
      counter.textContent = 'BATTLE ENDED';
      counter.style.color = 'rgba(220, 220, 220, 0.4)';
      counter.style.textShadow = 'none';
      setTimeout(() => {
        counter.textContent = 'CLICKS: 0/3';
  ...
  handleMouseMove(e) {
    ...
    // Create impact effects during battle
    if (this.state.isBattle && Math.random() < 0.02) {
      this.createImpactEffect(e.clientX, e.clientY);
    }
  },
  ...
  createImpactEffect(x, y) {
    ...
    impact.style.cssText = `
      ...
      pointer-events: none;
      z-index: 50;
      animation: errl-impact-fade 0.5s ease-out forwards;
    `;
  ...
  animationLoop() {
    ...
    // Update battle timer display if in battle
    if (this.state.isBattle) {
      this.updateBattleTimer();
    }

    const rect = errlWrapper.getBoundingClientRect();
    const centerX = rect.left + rect.width / 2;
    const centerY = rect.top + rect.height / 2;

    // Update each shard
    Object.entries(this.state.shards).forEach(([key, shard]) => {
      const config = this.shardConfigs[key];
      if (!config) return;

      if (this.state.isBattle && config.aggressive) {
        // Chase cursor with escalating intensity
        const dx = this.state.mouseX - centerX;
        const dy = this.state.mouseY - centerY;
        const distance = Math.sqrt(dx * dx + dy * dy);
        const maxDistance = 200 * this.state.intensity;
        const force = Math.min(distance / (50 / this.state.intensity), maxDistance);
...
function initBattle() {
  const errlElement = document.getElementById('errl');
  if (errlElement) {
    console.log('ErrlBattle: Initializing...');
    ErrlBattle.init();
  } else {
    // Retry if element not found yet
    setTimeout(initBattle, 100);
  }
}
