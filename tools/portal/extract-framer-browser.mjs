...
async function extractWithBrowser(sourcePath, targetSlug) {
  ...
  page.on("response", async (response) => {
    ...
    if (u.includes("framerusercontent.com") && u.includes(".mjs")) {
      try {
        const content = await response.text();
        scripts.set(u, content);
        console.log(`  📦 Captured: ${u.split("/").pop()} (${content.length} bytes)`);
      } catch (err) {
  ...
  if (!componentCode && scripts.size > 0) {
    ...
    let largestSize = 0;
    for (const [u, content] of scripts.entries()) {
      if (content.length > largestSize) {
        largestSize = content.length;
        largestScript = { url: u, content };
      }
    }
  ...
  const title = targetSlug.replace(/-/g, " ").replace(/\b\w/g, c => c.toUpperCase());
  const htmlContent = `<!doctype html>
<html lang="en">
  <head>
    <meta charset="utf-8">
    <meta name="viewport" content="width=device-width, initial-scale=1">
    <title>${title} — ${hasCanvas ? "Module" : "Component"}</title>
