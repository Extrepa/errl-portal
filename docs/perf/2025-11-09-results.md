...
- `src/styles.css`: removed `mix-blend-mode: screen` from `.errl-goo`, `.errl-aura-mask`, and `.errl-pool` (large gradient layers) to keep Safari from allocating duplicate full-screen backings; retuned gradient colors/opacity so the glow still reads against the dark background.
