...
## 🛠️ New Safety Commands

All automation lives under `tools/portal/` and is exposed via npm scripts:
...
### `npm run portal:checkpoint "message"`
Creates a safety checkpoint:
- Commits current work
- Creates backup branch
- Easy to revert if needed

### `npm run portal:visual-test`
...
- Usage: `npm run portal:rollback` or `npm run portal:rollback -- HEAD~2`
...
- ✅ Page-specific tests (about, gallery, assets)
...
# 2. Create checkpoint
npm run portal:checkpoint "about to add particle effects"
...
# 5. Test it
npm test
npm run portal:dev  # Test in browser
...
3. **Try it:** Run `npm run portal:safety-check`
...
**Next Steps:**
1. Run `npm run portal:safety-check` right now
...
npm run portal:safety-check          # Before changes
npm run portal:checkpoint "message"  # Create save point  
